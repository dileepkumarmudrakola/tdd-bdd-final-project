...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_invalid_available(self):
        """It should raise DataValidationError when deserializing invalid boolean for available"""
        product = ProductFactory()
        product_data = product.serialize()
        product_data["available"] = "invalid_boolean"
        new_product = Product()
        with self.assertRaises(DataValidationError) as context:
    ...
    def test_deserialize_invalid_attribute(self):
        """It should raise DataValidationError when deserializing with an invalid attribute"""
        product = ProductFactory()
        product_data = product.serialize()
        product_data["category"] = "NON_EXISTING_CATEGORY"
        new_product = Product()
        with self.assertRaises(DataValidationError) as context:
            new_product.deserialize(product_data)
    ...
    def test_deserialize_missing_key(self):
        """It should raise DataValidationError when deserializing with a missing key"""
        product = ProductFactory()
        product_data = product.serialize()
